...
const state = { category: catalog[0], selected: null, cart: [], location: null, beforePhoto: null };
...
$("#item-search").addEventListener("input", (event) => {
  ...
  const matches = catalog.flatMap((category) => category.items).filter((item) => item.name.toLowerCase().includes(query));
...
function renderPaymentGuidance() { const selected = document.querySelector("input[name=payment]:checked"); $("#payment-notice").textContent = paymentGuidance[selected.value]; }
...
$("#report-form").addEventListener("submit", async (event) => {
  ...
  try {
    const response = await fetch("/api/reports", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ address: $("#address").value.trim(), addressDetail: $("#address-detail").value.trim(), paymentMethod: payment.value, location: state.location, items: state.cart, beforePhoto: state.beforePhoto }) });
    const result = await response.json();
    if (!response.ok) throw new Error(result.error);
    const paymentLabel = payment.parentElement.innerText.split("\n")[0];
    message.textContent = `${result.reportNo}번으로 접수되었습니다. ${paymentLabel} 절차를 완료하면 수거 배정이 진행됩니다. 이 접수번호를 저장해두면 아래 "내 신고 확인하기"에서 수거 완료 사진을 볼 수 있어요.`;
  } catch (error) {
